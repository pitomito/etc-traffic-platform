...
- **`api_server_3.py`** — 服務主體，關鍵設計：
  ...
  - **分區結構相容**：M03A 資料湖正從日分區遷移到月分區，路徑產生邏輯
    ...
    前後都能查，不需要配合切換時間點改版。
- **`Dockerfile`** / **`pair2_api-deploy.yaml`** / **`pair2web-svc.yaml`** —
...
- **`webapp/`** — 這裡掛載的是「教學／展示用的獨立站台」靜態前端副本
  ...
  [`pair3_webapp/`](../pair3_webapp/)（Flask 代理版）是兩條並存的服務線，共用同一套
  查詢 API 後端，但各自持有一份前端靜態檔）。

## 與其他目錄的關係

```
pair2_convert/  →  HDFS /dataset（資料湖）
                                  │
                                  ▼
                         pair2_api/api_server_3.py（本目錄，直查資料湖）
                                  │  /api/*
                    ┌─────────────┴─────────────┐
                    ▼                             ▼
    pair3_webapp/（Flask 代理 + 正式站前端）   pair2_api/webapp/（教學站前端，FastAPI 直接掛載）
```
