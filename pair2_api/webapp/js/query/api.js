...
export async function parseJsonResponse(response) {
  ...
  try {
    ...
    throw new Error(`Flask 回傳內容不是合法 JSON：${text.slice(0, 500)}`);
  }
}
...
export async function sendQueryToFlask(query) {
  ...
  destroyM06ACharts();
  showResultMessage("loading", "查詢中，正在向 Flask 與資料查詢服務取得資料……");
  ...
  try {
    ...
    console.log("Flask API 回傳資料：", data);
    ...
    showResultMessage(
      "error",
      `查詢失敗：${error.message}。請確認 Flask 已啟動、後端查詢服務可連線，或展開下方技術資訊查看詳細內容。`
    );
  } finally {
