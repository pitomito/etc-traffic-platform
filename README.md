...
**查詢 API（`pair2api/api_server_3.py`）**
...
- **門架新舊編號自動展開**：真實世界中門架會改編號，跨切換日的查詢若只用單一編號會漏抓、
  ...
  確保同一實體路段的資料完整且連續（詳見 `pair2api/CHANGES_2026-07-17_gantry_alias.md`）。
...
```
 政府開放資料 (每日 CSV / tar.gz)
   ...
   HDFS /raw  ──Spark 轉檔──▶  HDFS /dataset（年月分區 Parquet 資料湖）
              ...
              ┌─────────────────────────┘
              ▼
   FastAPI + Spark（pair2api）  ◀── 直查資料湖，無中間 DB
              │  /api/*
              ▼
   Flask 代理 + 靜態前端（webapp）  ◀── 使用者：選路段、選時段
              │
              ▼
...
python pair2api/api_server_3.py        # 監聽 :8000，直查 HDFS 資料湖
```
