...
# 一定要用絕對路徑：dtadm pod 的 /home/bigred 非持久，重啟後 ~/bin 的
# spark-submit 連結就沒了，kubectl exec 的 PATH 裡找不到（2026-07-17 踩到）。
SPARK_SUBMIT = (
    "/opt/zfs/sys/spark-3.4.4-bin-hadoop3/bin/spark-submit "
...
#   注意要明確 export STAGING_BASE：put_to_hdfs.sh 預設用 $HOME 推 staging 路徑，
#   kubectl exec 進來是 root（$HOME=/root），會找錯地方（2026-07-17 踩到）。
PUT_REMOTE = (
    f'export STAGING_BASE={STAGING_ROOT}; '
...
#   convert_batch.py --auto 掃 D-5~D-24（2026-07-17 起直接寫 /dataset/，不再走 Iceberg）:
#     M03A 逐日（day= 目錄存在=已轉）; M06A 逐月（月目錄的 _raw_count=N 戳記
#     跟 raw 檔數比對,不同就整月重轉——當月每天重轉一次是預期行為）。
...
with DAG(
    ...
    # 轉 Parquet（raw → /dataset/）—— 2026-07-21 暫時從 DAG 拔掉，改人工執行。
    #   原因：節點 pids-limit 重啟後退回 2048，Spark 一跑 globPath 就 native-thread
    #   OOM、convert task 僵死拖住整條 DAG。DAG 現在只負責 fetch/put/heal 三段，
    #   轉檔改由人工 `convert_batch.py --auto`（M06A/M03A）在 pids-limit 拉高後手動跑。
    #   要復原：解除下面註解，並把依賴鏈末端接回 >> convert_parquet。
    # convert_parquet = BashOperator(
    #     task_id="convert_parquet",
    #     bash_command=wrap_remote(CONVERT_REMOTE),
    # )

    fetch_csv >> put_to_hdfs >> verify_and_heal
